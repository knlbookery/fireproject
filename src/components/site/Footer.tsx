...
export function Footer() {
  return (
    <footer className="w-full bg-surface px-6 pb-10 pt-16 text-ink lg:px-10 lg:pt-20">
      <div className="mx-auto max-w-[1400px]">
          ...
          <div className="grid gap-10 sm:grid-cols-3">
            {COLUMNS.map((col) => (
              <nav key={col.heading} aria-label={col.heading}>
                <h2 className="text-[0.7rem] font-medium uppercase tracking-[0.24em] text-ink/50">
                  {col.heading}
                </h2>
                <ul className="mt-4 space-y-2.5 text-sm">
                  {col.links.map((l) => (
                    <li key={l.to}>
                      <Link
                        to={l.to}
                        className="text-muted-foreground transition-colors hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                      >
                        {l.label}
                      </Link>
                    </li>
        ...
        <div className="flex flex-col gap-4 pt-8 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs">
            ...
            <Link to="/privacy-policy" className="text-muted-foreground underline-offset-4 hover:underline">
              Privacy Policy
            </Link>
            <Link to="/terms-of-use" className="text-muted-foreground underline-offset-4 hover:underline">
              Terms of Use
            </Link>
          </div>
        </div>
